...
graphDesignerMouseEvents model =
    let
        createNewNodeOnClick = case model.linkPreview of
            True -> [ mouseEvent "mouseup" ( \i -> InterruptLinkPreview ) ]
            False -> [ mouseEvent "click" CreateNode ]
        updateLinkPreviewOnMove = case model.linkPreview of
            True -> [ mouseEvent "mousemove" LinkPreviewPointerMoved ]
            False -> []
    in
...
circleMouseEvents model id = 
    let
        putFocusOnNodeEvent = case model.linkPreview of
            False -> [ mouseEvent "mousedown" ( \i -> (FocusOnNode id) ) ]
            True -> [ ]
        insertNewLinkEvent = case ( model.linkPreview, model.focussedNode ) of
            (True, Just sourceId) -> [ mouseEvent "mouseup" ( \i -> (CreateLinkFromTo sourceId id)) ]
            other -> []
    in
        putFocusOnNodeEvent ++ insertNewLinkEvent
